...
class Pagination {
  ...
  async getEdges(queryArgs) {
    ...
    if (!first && !last) {
      ...
      throw new UserInputError(
        "Passing `first` and `last` arguments is not support with this connection."
      );
    } else if (first < 0 || last < 0) {
      throw new UserInputError(
        "Minium record request for `first` and `last` arguments is 0."
      );
  ...
  // direction of the set order (e.g. next/previous page checks)
