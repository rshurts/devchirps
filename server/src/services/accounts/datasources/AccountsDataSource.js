  ...
  async updateAccount(id, { email, newPassword, password }) {
    if (!email && !newPassword && !password) {
      ...
      throw new UserInputError(
        "Email and passowrd cannot be updated simultaneously."
      );
    ...
    // If no email is provided then it's a password update.
    // Get the token using the credentials to verify the user old password is correct.
    if (!email) {
      const user = await this.auth0.getUser({ id });
    ...
    // Otherwise this is an email update.
